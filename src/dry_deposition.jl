...
fractions (`f_urban`, `f_agricultural`, ..., `f_rockyshrubs`) as computed
variables instead of a single dominant `landuse` index. Vd is computed by
...
(vegetated-land fallback). PR 2 backs it with a NetCDF of MODIS-derived
Wesely-class fractions.
"""
